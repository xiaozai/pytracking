...
def run(settings):
    ...
    settings.description = 'ATOM IoUNet with default settings, but additionally using CDTB color for training, with depth masking.'
    ...
    dataset_train = sampler.ATOMSampler_depth_mask([cdtb_train], [1],
                                samples_per_epoch=1000*settings.batch_size, max_gap=50, processing=data_processing_train)
    ...
    # The sampler for validationactors
    # dataset_val = sampler.ATOMSampler([got10k_val], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,
    #                                   processing=data_processing_val)
    dataset_val = sampler.ATOMSampler_depth_mask([cdtb_val], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,
                                      processing=data_processing_val)
