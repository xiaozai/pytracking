...
def run(settings):
    ...
    # net = detr.build_tracker(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
    ...
    # objective = {'iou': nn.MSELoss(), 'conf': nn.MSELoss()}
    objective = {'bboxes': SetCriterion()} # Song wants the giou loss in Detr

    loss_weight = {'iou': 1, 'conf': 100}
