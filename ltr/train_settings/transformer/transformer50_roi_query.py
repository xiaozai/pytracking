...
from ltr.models.transformer import detr_roi, detr_roi_query, detr
...
def run(settings):
    settings.description = 'Default train settings for Transformer with ResNet50 as backbone, and  PrROIPooling + Query from template branch.'
    ...
    # Train sampler and loader # Song, ignore Tracking-net currently, trackingnet_train
    ...
    net = detr_roi_query.build_tracker(backbone_name='resnet50',
    ...
    param_dicts = [
        {"params": [p for n, p in net.named_parameters() if "template_backbone" not in n and "search_backbone_pos" not in n and p.requires_grad]},
        {
            "params": [p for n, p in net.named_parameters() if "search_backbone_pos.1" in n and p.requires_grad],
            "lr": 1e-5,
        },
    ]
