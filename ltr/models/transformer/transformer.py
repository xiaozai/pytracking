...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        # print('song in transformer.py: line 106, memory.shape = ', memory.shape)
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed)
        # print('song hs shape = ', hs.shape)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        # print('Song in TransformerEncoderLayer forward_post, src.shape : ', src.shape)
        src2 = self.self_attn(q, k, value=src, attn_mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]
        # print('Song in TransformerEncoderLayer forward_post, src2.shape : ', src2.shape)
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        # print('Song in 211, src2.shape')
