...
class CDTB_color_depth_mask(BaseVideoDataset):
    ...
    def _get_depth_path(self, seq_path, frame_id):
        return os.path.join(seq_path, 'depth/{:08}.png'.format(frame_id+1))     # Song : get corresponding depth

    def _get_frame(self, seq_path, frame_id):
        return self.image_loader(self._get_frame_path(seq_path, frame_id))      # (H, W, 3)

    def _get_depth(self, seq_path, frame_id):
        return cv2.imread(self._get_depth_path(seq_path, frame_id), -1)         # Song :  (H, W) get coresponding depth
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        '''
        Song : the only difference is to return corresponding depths images as well
        '''
        seq_path = self._get_sequence_path(seq_id)
        # obj_meta = self.sequence_meta_info[self.sequence_list[seq_id]]
        obj_class = self._get_class(seq_path)

        frame_list = [self._get_frame(seq_path, f_id) for f_id in frame_ids]    # (N, H, W, 3)
        depth_list = [self._get_depth(seq_path, f_id) for f_id in frame_ids]    # (N, H, W)
