...
class CDTBRGBDDataset(BaseDataset):
    """
    CDTB, RGB + Depth dataset
    """
    def __init__(self):
        super().__init__()
    ...
    def _construct_sequence(self, sequence_name):
        ...
        frames_rgb = ['{base_path}/{sequence_path}/color/{frame:0{nz}}.{ext}'.format(base_path=self.base_path,
                  sequence_path=sequence_path, frame=frame_num, nz=nz, ext=ext)
                  for frame_num in range(start_frame, end_frame+1)]
        frames_depth = ['{base_path}/{sequence_path}/depth/{frame:0{nz}}.png'.format(base_path=self.base_path,
        ...
        return Sequence(sequence_name, frames_rgb, 'cdtb_rgbd', ground_truth_rect, depth_frames=frames_depth)
